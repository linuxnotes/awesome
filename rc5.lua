...
function l2file(text) 
   file = io.open("log.rst", "a+")
   file:write(text .. "\n")
   file:close()
end
...
langwidget = wibox.widget.textbox()
...
-- cpuwidget = wibox.widget.graph({layout=awful.widget.layout.horizontal.rightleft})
...
--     }
	...
	local right_layout = wibox.layout.fixed.horizontal()
	if s == 0 then right_layout:add(wibox.widget.systray()) end
